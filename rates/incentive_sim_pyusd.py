#!/usr/bin/env python3
"""Supply-sink incentive testbed, depositor dynamics CALIBRATED from the pyUSD fit.

This is the `incentive_sim.py` testbed re-parameterised with the depositor-response
model measured in the unified pyUSD/crvUSD TVL fit (REPORT_pool_dynamics.md):

  * dead band [1.50x, 2.14x] market (inflow edge 2.14x, was a flat 2x guess)
  * tau_out 5.9 d (outflow); tau_in base 30 d
  * **rush deposits**: inflow rate scales with (x/HYST)^p_in, p_in ~ 1.03 — so a
    high-APR burst pulls capital in ~(x/HYST)x FASTER than a fixed-tau model. This
    is measured evidence for the burst strategy: offer a very high APR briefly and
    the sink fills fast (the "tiny-pool rush").

Idea: when LevAMM runs a positive net pressure (crvUSD shortfall), pay a temporary
bonus APR on scrvUSD; depositors arrive and the incremental sink S absorbs it. Cover
positive pressure P at minimal spend. All in **normalised units** (P, S as fractions
of half-TVL; multiply by half-TVL for $). Market norm = Aave USDC APR (7-day EMA).

Model
-----
  signal      P(t) = max(0, net_pressure)                       [frac of half-TVL]
  controller  S*(t) = f(P, S, state)        (the sink we aim to attract)
  offer       x(t) = HYST + S*/beta         (advertised APR as a multiple of market;
              must clear the HYST=2.14x dead-band; beta = sink per unit excess ratio)
  dynamics    inflow:  dS/dt = (S*-S)/tau_in · (x/HYST)^p_in   (rush)
              outflow: dS/dt = (S*-S)/tau_out
  spend rate  (x-1)*m*S      (bonus APR above base 1x market, paid on the sink S)

Cost J = spend + lambda * undercoverage_area.

Usage
-----
    uv run python incentive_sim_pyusd.py --controller pid --optimize
    uv run python incentive_sim_pyusd.py --controller pid --sweep-scap --buffer 0 --eval-reserve 0.20
"""
import argparse
import lzma
...
YEAR = 365.25 * 86400.0
# Depositor-response dynamics calibrated from the pyUSD/crvUSD unified TVL fit.
TAU_IN = 30.0 / 365.25    # years (base inflow near the threshold; rush-dominated)
TAU_OUT = 5.9 / 365.25    # years (outflow)
HYST = 2.14               # dead-band inflow edge: need 2.14x market APR to attract
X_LO = 1.50               # dead-band outflow edge (reference)
P_IN = 1.03               # rush: inflow rate scales with (x/HYST)^P_IN (tiny-pool appeal)
...
def simulate(P, m, dt_year, ctrl_name, params, beta=BETA, scap=SCAP,
    ...
    for k in range(n):
        ...
        if st > s:                           # inflow — rush: faster the higher the offer
            frac = (dt_year / tau_in) * (xk / HYST) ** p_in
            s = s + (st - s) * min(frac, 1.0)
        else:                                # outflow — sink drains at tau_out
            s = s + (st - s) * (dt_year / tau_out)
        Star[k] = st
        S[k] = s
        if st > 0.0:
            x[k] = xk
...
def plot(grid, P, m, m_raw, sim, mets, ctrl_name, params, save=None):
    ...
    ax.axhline(HYST, color="k", ls="--", lw=0.8, label=f"{HYST:.2f}x dead-band")
