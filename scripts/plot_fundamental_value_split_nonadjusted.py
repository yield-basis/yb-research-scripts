...
def main():
        ...
        for block in range(START_BLOCK, current_block - (BATCH_SIZE - 1), BATCH_SIZE):
            ...
            for from_block, to_block in zip(blocks[:-1], blocks[1:]):
                ...
                if to_block > from_block:
                    ...
                    d_profit = to_value_scale - from_value_scale
                    ...
                    d_staked_value = 0
                    d_unstaked_value = 0
                    useful_value = to_value_scale * to_liquidity[1] / (to_liquidity[0] + to_liquidity[1])
                    new_staked_pps = None
                    if staker_supply > 0:
    ...
    for idx in range(n):
        ax_rel.plot(times[idx], growth_scale_values[idx], label=labels[idx], c=colors[idx])
